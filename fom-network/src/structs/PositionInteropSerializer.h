...
class PositionInteropSerializer
    : protected InteropTypeSerializer<PositionInterop> {
 public:
  explicit PositionInteropSerializer(uint32_t precision = 16)
      : precision_(precision) {}

  void Write(RakNet::BitStream& bs, const PositionInterop& data) const {
    if (precision_ > 15) {
  ...
  bool Read(RakNet::BitStream& bs, PositionInterop& data) const {
    if (precision_ > 15) {
      uint16_t x, y, z;
