...
#include <fom-network/structs/ApartmentInterop.h>

#include "InteropTypeSerializer.h"
#include "item/ItemListInteropSerializer.h"

namespace FOMNetwork {

class ApartmentInteropSerializer
    : protected InteropTypeSerializer<ApartmentInterop> {
 public:
  void Write(RakNet::BitStream& bs, const ApartmentInterop& data) const {
  ...
  bool Read(RakNet::BitStream& bs, ApartmentInterop& data) const {
    ItemListInteropSerializer itemListSerializer;
    uint8_t skipU8;
    uint32_t skipU32;
