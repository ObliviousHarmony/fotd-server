...
#include "../types/world/WorldObjectSerializer.h"
#include "../types/world/WorldServiceControlSerializer.h"
#include "../types/world/WorldServiceSerializer.h"
...
void WriteObjects(RakNet::BitStream& bs,
                  const Type::WorldObjectSerializer& serializer,
                  const Type::WorldObject* objects, uint32_t count) {
...
void WriteServices(RakNet::BitStream& bs,
                   const Type::WorldServiceSerializer& serializer,
                   const Type::WorldService* services, uint32_t count) {
...
void WriteControls(RakNet::BitStream& bs,
                   const Type::WorldServiceControlSerializer& serializer,
                   const Type::WorldServiceControl* controls, uint32_t count) {
...
void WorldObjectsSerializer::Write(RakNet::BitStream& bs,
  ...
  Type::WorldObjectSerializer objectSerializer;
  Type::WorldServiceSerializer serviceSerializer;
  Type::WorldServiceControlSerializer controlSerializer;

  bs.WriteCompressed(data->action);

  switch (data->action) {
    case WORLD_OBJECTS_ACTION_WORLD_SYNC: {
      const WorldObjects::WorldSync& sync = data->sync;
