...
{
    [PacketHandler]
    internal class MoveItemsPacketHandler : PacketHandlerBase<MoveItemsPacket>
    {
        private readonly IPlayerRegistry _playerRegistry;
        private readonly IClientPacketSender _clientPacketSender;
        private readonly ILogger<MoveItemsPacketHandler> _logger;

        private readonly Dictionary<TransferKey, TransferRule> _transferRules;

        public MoveItemsPacketHandler(
            IPlayerRegistry playerRegistry,
            IClientPacketSender clientPacketSender,
            ILogger<MoveItemsPacketHandler> logger
        )
        {
        ...
        public override void Handle(NetworkAddress sender, in MoveItemsPacket p)
        {
            ...
            using var response = new PacketWriter<MoveItemsPacket>(sender);
            response.Data = p;
        ...
        private string? ValidateItemContainers(Player player, in MoveItemsPacket p)
        {
        ...
        private bool MoveBetweenContainers(Player player, in MoveItemsPacket p)
        {
        ...
        private bool MoveWeaponBetweenSlots(Player player, in MoveItemsPacket p)
        {
            var fromContainer = GetItemContainer(player, p.From, p.FromSlot);
            var toContainer = GetItemContainer(player, p.To, p.ToSlot);
        ...
        private bool MoveQuickslots(Player player, in MoveItemsPacket p)
        {
            uint? itemId = null;
            if (p.NumItemIds > 0)
            {
